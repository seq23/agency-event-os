...
for (const token of ["eventIdFromPath", "eventIdsMatch(pathEventId, payload.eventId)", "canPerformCrewAction", "assertCanPerformCrewAction"]) {
  if (!auth.includes(token)) throw new Error(`Missing route authorization invariant: ${token}`);
}
for (const token of ["event-12", "role escalation", "/app/events/event-1", "/sponsor/events/event-1"]) {
