...
export function SpeakerApprovalQueue({ eventId }: { eventId: string }) {
  const items = getEventApprovalQueue(eventId).filter((item) => item.itemType.startsWith("speaker_"));

  return (
    <SectionCard title="Speaker approval queue" eyebrow="Assets, scripts, decks">
