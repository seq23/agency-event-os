...
const logoAsset = path.join(root, 'public/brand/west-peek-productions-logo.jpg');
if (!fs.existsSync(logoAsset)) failures.push("approved West Peek logo asset is missing: public/brand/west-peek-productions-logo.jpg");
const logoReferences = ['components/brand/WestPeekProductionsLogo.tsx', 'app/layout.tsx'];
if (!logoReferences.some((file) => {
  const full = path.join(root, file);
  return fs.existsSync(full) && fs.readFileSync(full, "utf8").includes('WestPeek');
