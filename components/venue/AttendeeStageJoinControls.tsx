...
export function AttendeeStageJoinControls({ eventId, roomId, control, capability, attendeeId }: { eventId: string; roomId: string; control: AttendeeLiveControlState; capability?: AttendeeLiveCapability; attendeeId?: string }) {
  if (!attendeeId) return <div className="rounded-2xl bg-slate-100 p-4 text-sm font-bold text-slate-700" data-testid="stage-join-registration-required">Register to enter the live stage. Guest registration does not publish by default.</div>;
