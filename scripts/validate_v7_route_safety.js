...
for (const [name, content] of [['crew', crew], ['special guest', guest]]) {
...
for (const [name, content] of [['launchpad', launchpad], ['operator packet', packet]]) {
  if (!content.includes('readV5AccessCookie') || !content.includes('payload.kind === "crew"')) throw new Error(`${name} must require a valid crew access cookie.`);
  if (!content.includes('redirect("/production-access/crew')) throw new Error(`${name} must redirect unauthenticated users to the crew gate.`);
