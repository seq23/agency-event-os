...
export default function JoinEventPage({ searchParams }: { searchParams?: { code?: string } }) {
  const resolution = resolveEventJoinCode(searchParams?.code);
  return (
    <main className="min-h-screen bg-brand-ash px-5 py-10 text-brand-black sm:px-8 lg:px-12">
      <section className="mx-auto max-w-3xl rounded-[2rem] border border-brand-line bg-white p-6 shadow-brand sm:p-10">
        <p className="text-xs font-black uppercase tracking-[0.35em] text-brand-orange">Attendee access</p>
        ...
        <form className="mt-6 flex flex-col gap-3 sm:flex-row" action="/join" method="get">
          <input name="code" defaultValue={searchParams?.code ?? ""} aria-label="Event code" className="min-h-12 flex-1 rounded-full border border-brand-line px-5 text-sm outline-none focus:border-brand-orange" />
