    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm" data-testid="attendee-live-control-panel">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <form action={updateAttendeeLiveControl} className="rounded-2xl bg-slate-50 p-4">
          ...
          <label className="mt-2 block text-sm"><input name="requestRequired" type="checkbox" defaultChecked={main.requestRequired} /> Require crew approval</label>
      ...
      <form action={setAttendeeLiveApproval} className="mt-4 rounded-2xl border border-slate-200 p-4">
        <p className="font-black">Attendee approval / revoke path</p>
        <input type="hidden" name="eventId" value={eventId} /><input type="hidden" name="roomKind" value="main_stage" /><input type="hidden" name="roomId" value="main-stage" />
        ...
        <label className="mt-3 inline-flex items-center gap-2 text-sm"><input name="approvedForStage" type="checkbox" /> Approve for main stage</label>
        ...
        <label className="ml-4 inline-flex items-center gap-2 text-sm"><input name="revoked" type="checkbox" /> Revoke</label>
        <button className="ml-4 rounded-full border border-slate-300 px-4 py-2 text-xs font-black">Apply attendee permission</button>
      </form>
    </section>
  );
}
